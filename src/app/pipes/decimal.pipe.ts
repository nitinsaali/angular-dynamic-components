...
DecimalFormat.prototype.format = function(numStr) { // 1223.06 --> $1,223.06
    ...
    for (i=intStr.length; i<minInt; i++) { // if minInt=4 then 034 --> 0034
        intStr = '0' + intStr;
    }
...
export class DecimalPipe implements PipeTransform {
  ...
  transform(value: number, formatter?: string): any {
    if (isNumber(value)) {
      const decimal: any = new DecimalFormat(formatter);
